...
double compute_cross_sec_param(const VarMap& var_map) {
  ...
  auto beta = 1./proton_ivar/(1.+width*width/corr_l/corr_l);
  try {
	...
	// <<< Weiyao
	//std::cout << "Sigma_gg = " << std::exp(.5*(result.first + result.second))*(4 * math::double_constants::pi * sqr(width)) << std::endl;
